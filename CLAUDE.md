...
- **Releases:** push a `v*` tag → `.github/workflows/release.yml` ships a universal,
  signed, notarized, stapled `.pkg` that installs `FT8-808.app` → `/Applications` and the
  `ft8term` CLI → `/usr/local/bin`. The `.app` is assembled by `Scripts/build-app-bundle.sh`
  (embeds the Hamlib dylib in `Contents/Frameworks` via an `@executable_path/../Frameworks`
  rpath); its icon is generated by `Scripts/make-appicon.swift`. See `docs/releasing.md`.
  Note: the universal build is per-arch + `lipo` (a simultaneous `--arch arm64 --arch x86_64`
  build can't resolve the Hamlib xcframework's library search path under SwiftPM).
