...
drops **`FT8-808.app`** (the native SwiftUI app) into `/Applications` and the
**`ft8term`** CLI (with `libhamlib.4.dylib` beside it) into `/usr/local/bin`. The
app carries its own copy of the Hamlib dylib in `Contents/Frameworks`.
...
The workflow rebuilds Hamlib universal, builds `ft8term` and `ft8gui` for both
arches, assembles `FT8-808.app` (`Scripts/build-app-bundle.sh`), signs +
...
Double-click the `.pkg` → it installs **`FT8-808.app`** into `/Applications` and
**`ft8term`** into `/usr/local/bin`. Because it's notarized and stapled,
Gatekeeper clears it with no quarantine prompt, even offline. Then either:

- launch **FT8-808** from `/Applications` (or Spotlight) for the native app, or
- run `ft8term` in a terminal for the TUI.

Both share the same `~/.config/ft8-808/config.json`.

## Notes

- The runner is `macos-14` (Apple Silicon); it cross-builds the x86_64 slice.
- `Scripts/ft8term.entitlements` / `Scripts/ft8gui.entitlements` grant the
  microphone entitlement that live capture needs under the hardened runtime; the
  usage strings live in `Sources/ft8term/Info.plist` (embedded in the binary) and
  `Sources/ft8gui/Info.plist` (the app bundle's `Contents/Info.plist`).
- `Scripts/build-app-bundle.sh` assembles `FT8-808.app`: it copies the universal
  `ft8gui` to `Contents/MacOS/FT8-808`, embeds `libhamlib.4.dylib` in
  `Contents/Frameworks` (found via an `@executable_path/../Frameworks` rpath),
  writes `Contents/Info.plist` + the icon, and (given an identity) signs the dylib
  then the bundle.
- The app icon is generated by `Scripts/make-appicon.swift` → committed
  `Sources/ft8gui/AppIcon.icns`. Re-run it (from the repo root) to change the art.
